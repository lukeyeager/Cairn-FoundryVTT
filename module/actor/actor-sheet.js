 ...
 * @extends {ActorSheet}
 */
export class MonolithActorSheet extends ActorSheet {
  /** @override */
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["monolith", "sheet", "actor"],
      template: "systems/monolith/templates/actor/actor-sheet.html",
      width: 480,
      height: 640,
      tabs: [
        {
      ...
      dragDrop: [{ dragSelector: ".monolith-items-list-row", dropSelector: null }],
    });
  }

  get template() {
    const path = "systems/monolith/templates/actor";
  ...
  activateListeners(html) {
    ...
    html.find(".item-edit").click((ev) => {
      const li = $(ev.currentTarget).parents(".monolith-items-list-row");
  ...
  async _onItemCreate(event) {
    ...
    const template = "systems/monolith/templates/dialog/add-item-dialog.html";
    const content =  await renderTemplate(template);

    new Dialog({
      title: game.i18n.localize("MONOLITH.CreateItem"),
      content,
      buttons: {
        create: {
          icon: '<i class="fas fa-check"></i>',
          label: game.i18n.localize("MONOLITH.CreateItem"),
          callback: (html) => {
  ...
  async _onAddFatigue(event) {
    ...
    this.actor.createOwnedItem({
      name: game.i18n.localize("MONOLITH.Fatigue"),
      type: 'item'
    });
  }
  ...
  async _onRemoveFatigue(event) {
    ...
    const fatigues = this.actor.items
      .filter(i => i.name === game.i18n.localize("MONOLITH.Fatigue"));
  ...
  async _onRoll(event) {
    ...
    if (dataset.roll) {
      ...
      const label = dataset.label ? game.i18n.localize("MONOLITH.RollingDmgWith") + ` ${dataset.label}` : "";
  ...
  _buildDamageRollMessage(label, targetIds) {
    const rollMessageTpl = 'systems/monolith/templates/chat/dmg-roll-card.html';   
  ...
  _onItemDescriptionToggle(event) {
    event.preventDefault();
    const boxItem = $(event.currentTarget).parents(".monolith-items-list-row");
  ...
  async _onRollAbility(event) {
    ...
    if (dataset.roll) {
      const roll = await evaluateFormula(dataset.roll, this.actor.getRollData());
      const label = dataset.label ? game.i18n.localize("MONOLITH.Rolling") + ` ${dataset.label}` : "";
      const rolled = roll.terms[0].results[0].result;
      const result = roll.total === 0 ? game.i18n.localize("MONOLITH.Fail") : game.i18n.localize("MONOLITH.Success");
  ...
  async _onRegenerateCharacter(event) {
    ...
    const confirm = await Dialog.confirm({
      title: game.i18n.localize("MONOLITH.CharacterRegeneratorTitle"),
      content: `<p>${game.i18n.localize("MONOLITH.CharacterRegeneratorConfirm")}</p>`,
      defaultYes: false,
    });
      ...
      return [
        {
          ...
          label: game.i18n.localize("MONOLITH.RegenerateCharacter"),
          icon: "fas fa-skull",
