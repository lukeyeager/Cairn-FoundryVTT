 ...
 * @return {Promise.<void>}
 */
export const createMonolithMacro = async (data, slot) => {
  ...
  const command = `game.monolith.rollItemMacro("${actor.id}", "${item.id}");`;
...
export const rollItemMacro = async (actorId, itemId) => {
  ...
  const flavor = `${game.i18n.localize("MONOLITH.RollingDmgWith")} ${item.name}`;
