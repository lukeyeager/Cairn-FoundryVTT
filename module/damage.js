...
export class Damage {
    ...
    static _showDetails(data) {
        ...
        let content = '<p><strong>' + game.i18n.localize('MONOLITH.Damage') + '</strong>: ' + dmg + ' (' + damage + '-' + armor + ')</p>'
        if (newHp !== hp) {
            content += '<p><strong>' + game.i18n.localize('MONOLITH.HitProtection') + '</strong>: <s>' + hp + '</s> => ' + newHp + '</p>'
        } else {
            content += '<p><strong>' + game.i18n.localize('MONOLITH.HitProtection') + '</strong>: ' + hp + '</p>'
        }
        if (newStr !== str) {
        ...
        if (newStr < str) {
            if (newStr === 0) {
                content += '<strong>' + game.i18n.localize('MONOLITH.Dead') + '</strong>'
            } else  {
                content += '<p><strong>' + game.i18n.localize('MONOLITH.StrSave') + '</strong></p>'
                content += '<button type="button" class="roll-str-save">' + game.i18n.localize('MONOLITH.RollStrSave') + '</button>'
            }
        } else if (newHp === 0 && hp !== 0) {
            content += '<p><strong>' + game.i18n.localize('MONOLITH.Scars') + '</strong></p>'
    ...
    static async _rollScarsTable(damage){
        const table = await findCompendiumItem("monolith.utils", "Scars");
    ...
    static async _rollStrSave(actor,html){
        ...
        const label = game.i18n.format("MONOLITH.Save",{key: game.i18n.localize("STR")});
